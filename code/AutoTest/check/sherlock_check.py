...
from util import utils, sherlock_utils

def get_farthest_val_and_score(dist_val, label):
...
def sherlock_check_parallel(df, test_matching_dict, rule_list, n_proc):
    with mp.Manager() as manager:
        ...
        with mp.Pool() as pool:
            ...
            pool.starmap(sherlock_check_parallel_core, zip([ns] * n_proc, start_list, end_list, queue_list))
            results = []
            for q in queue_list:
                while not q.empty(): 
...
def get_all_outliers(dist_val, label, score_thres):
    y_pred, _, _ = sherlock_utils.predict([[v] for v in dist_val], model = None)
    label_idx = sherlock_utils.class_list.index(label)
    label_score = y_pred[:, label_idx]
...
def sherlock_all_outliers_parallel_core(ns, start, end, queue):
        df = pd.DataFrame(ns.df[start : end], columns = ns.df_col, index = ns.df_idx[start : end])
        rule_list = ns.rule_list
        test_matching_dict = ns.test_matching_dict
        for i in range(len(rule_list)):
            ...
            if len(matching_rows) == 0: continue
            matching_rows = matching_rows.assign(outlier = None, pre = None, outlier_score = -100, conf = -100, thres = -100, cohenh = -100, contingency = None)
            matching_rows['outlier'] = matching_rows['dist_val'].apply(lambda x: get_all_outliers(x, label, score_thres))
...
def sherlock_all_outliers_parallel(df, test_matching_dict, rule_list, n_proc):
    with mp.Manager() as manager:
        ns = manager.Namespace()
        ns.df = manager.list(df.values.tolist())
        ns.df_idx = manager.list(df.index.tolist())
        ...
        with mp.Pool() as pool:
            ...
            end_list = [len(df) * (i + 1) // n_proc for i in range(n_proc)]
            queue_list = [mp.Manager().Queue() for _ in range(n_proc)]
            pool.starmap(sherlock_all_outliers_parallel_core, zip([ns] * n_proc, start_list, end_list, queue_list))
            results = []
            for q in queue_list:
                while not q.empty(): 
                    results.append(q.get())
